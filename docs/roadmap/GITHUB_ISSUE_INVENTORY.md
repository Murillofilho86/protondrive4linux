...
| M1-001 | Disaster test suite | M1 | `type:test` `area:testing` `risk:data-loss` `priority:critical` | ✅ (disco cheio/timeout/corrida watch ficam fora, documentado) |
...
| M2-001 | Release signing | M2 | `type:security` `area:security` `priority:critical` | 🔶 (chave rekeyed 2026-09-13, `RELEASE_SIGNING_READY=true`, falta confirmar a 1ª release assinada) |
